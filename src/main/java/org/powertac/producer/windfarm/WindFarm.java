...
public class WindFarm extends Producer{
	...
	protected double getOutput(WeatherReport weatherReport) {
		return getPowerOutput(weatherReport.getTemperature(), weatherReport.getWindSpeed());
	}

	@Override
	protected double getOutput(int timeslotIndex,
			WeatherForecastPrediction weatherForecastPrediction) {
		return getPowerOutput(weatherForecastPrediction.getTemperature(), weatherForecastPrediction.getWindSpeed());
	}
}
