...
public abstract class Producer extends AbstractCustomer{
	 ...
	 * @param profileHours The number of hours to profile the plant output
	 * @param capacity the powerplant capacity in kw < 0
	 */
	public Producer(String name,PowerType powerType,int profileHours,double capacity) {
		super(name);

		if (capacity >= 0)
			throw new IllegalArgumentException("Positive plant capacity");
		
		//Initialize the weather report and forecast repositories because they are not included in the AbstractCustomer
	...
	public void consumePower (){
		...
		List<TariffSubscription> subscriptions = 
				tariffSubscriptionRepo.findActiveSubscriptionsForCustomer(getCustomerInfo().get(0));
	...
	public void subscribeDefault(){
		...
		if(subscriptions.size() > 0){
			...
			//We could search for tariff with Production power type but it isn't worth it to change the power type
			log.error("Got empty subscriptions list. We dont have an avtive tariff");
	}
	...
	public void evaluateNewTariffs(){
		...
		//check if the active tariff changed and recalculate the preferred output
		List<TariffSubscription> subscriptions = 
				tariffSubscriptionRepo.findActiveSubscriptionsForCustomer(getCustomerInfo().get(0));
		...
		if(subscriptions.size() > 0){
			if(subscriptions.get(0) != currentSubscription){
				...
				currentSubscription = subscriptions.get(0);
				preferredOutput = producerAccessor.generateOutput(currentSubscription.getTariff(),
	...
	private class ProducerAccessor implements CustomerModelAccessor
	{
		 ...
		 * Care: this doesn't make sense for the weather sensitive producers, they should ignore the
		 * suggested preferred output or set the hours to 24 which by itself isn't very helpful.
		 * @return
		 */
		protected PreferredOutput generateOutput(Tariff tariff,int hours){
			...
			// are greater the the tariff payments. Since production is not an interruptible power type
			// we don't bother checking for curtailment.
			
			
			// Instead we can modify the max plant preferred output and select the lowest one that
			...
			for(preferredOutput = 0; preferredOutput >= upperPowerCap;preferredOutput -= step*upperPowerCap){
