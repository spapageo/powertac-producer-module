...
public class WindTurbineTest {
	...
	public void dataCalculateWindAtAltitude() throws FileNotFoundException {
		...
		PrintWriter pw = new PrintWriter(new File("dataCalculateWindAtAltitude.txt"));
		...
		for(double altitude = 5; altitude <= 500; altitude++){
			...
			int i = 0;
			for(double z0 : zlist){
				double ua = WindTurbine.calculateUasterisk(refSpeed, refAltitude, f, z0, 0.4);
	...
	public void dataGetPowerOutput() throws FileNotFoundException{
		...
		double[] y = {0,0,0,66,166,288,473,709,1000,1316,1651,1860,1968,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000};
		...
		for(double speed = 1; speed < 25;speed+=0.1){
			double f = WindTurbine.calulcatef(22);
			double ua = WindTurbine.calculateUasterisk(speed, wt.getRefAltitude(), f, wt.getSurfaceRoughness(),0.4);
