...
public class IrradianceModelTest {
	...
	public void testIrradianceModel() throws FileNotFoundException{
		...
		for(double g=0; g < 24; g+=0.1){
			...
			if(sunAltitude >= 0){
				...
				double direct = IrradianceModel.getDirectIrradiance(sunAltitude, 1367.7, IrradianceModel.getT0(airmass, 3),
						IrradianceModel.getTr(airmass), IrradianceModel.getaw(airmass, 0.1, 300), 0.98);
				double diffuse = IrradianceModel.getDiffuseIrradiance(sunAltitude, 1367.7, IrradianceModel.getT0(airmass, 3),
						IrradianceModel.getTr(airmass), IrradianceModel.getaw(airmass, 0.1, 300),
						0.98, 0.98, IrradianceModel.getf(sunAltitude));
	...
	public void testGetIrradianceOnTiltedPlane() throws FileNotFoundException{
		...
		double sa = 60;
		double pa = 30;
		double max = IrradianceModel.getIrradiancOnTiltedPlane(direct, diffuse, SolarModel.getIncidenceAngle(sa, 180, 180, pa), sa, pa, 0.2);
		for(pa = 0; pa <= 90; pa++){
			double incid = SolarModel.getIncidenceAngle(sa, 180, 180, pa);
			double inc = IrradianceModel.getIrradiancOnTiltedPlane(direct, diffuse, incid, sa, pa, 0.2);
	...
	public void testDailyIrradianceOnTitledPlane() throws FileNotFoundException{
		...
		PrintWriter pw = new PrintWriter(new File("dataIrradianceOnTiltedPlane.txt"));
		PrintWriter pw2 = new PrintWriter(new File("dataIrradianceOnTiltedPlane2.txt"));
		...
		for(double tilt = 0; tilt <= 90; tilt+=1){
			...
			for(double time = 0;time < 24; time+=1){
				double sunaltitude = SolarModel.getSunAltitudeAngle(time, panelLatitude, day);
				if(sunaltitude >= 0){
					double sunazimuth = SolarModel.getSunAzinuthAngle(sunaltitude, day, panelLatitude, time);

					double inci = SolarModel.getIncidenceAngle(sunaltitude, sunazimuth, 180 , tilt);
					double airmass = IrradianceModel.getAirMass(sunaltitude);
					...
					double dir = IrradianceModel.getDirectIrradiance(sunaltitude, 1367, T0, Tr, aw, 0.95);
					double dif = IrradianceModel.getDiffuseIrradiance(sunaltitude, 1367, T0, Tr, aw, 0.95, 0.95, f);
					dir = IrradianceModel.getCloudModifiedIrradiance(dir, 0, 0.2, 0.95,0.33, -1.06,  0,  0);
					dif = IrradianceModel.getCloudModifiedIrradiance(dif, 0, 0.2, 0.95,0.33, -1.06,  0,  0);
					
					double res;
					if(inci <= 90){
						res = IrradianceModel.getIrradiancOnTiltedPlane(dir, dif, inci, sunaltitude, tilt, 0.2);
					}else{
						res = IrradianceModel.getIrradiancOnTiltedPlane(0, dif, inci, sunaltitude, tilt, 0.2);
//						System.out.println("Incidence:" + inci +"for tilt " + tilt + " and hour " + time);
//						System.out.println("Sun's posistion: " + sunaltitude + "," + sunazimuth);
					}
...
//					}
	...
	public void dataYearIrradianceOnTitledPlane() throws FileNotFoundException{
		...
		PrintWriter pw = new PrintWriter(new File("dataYearIrradianceOnTitledPlane.txt"));
		...
		for(int day = 1; day <= 365; day+=1){
			...
			for(double time = 0;time < 24; time+=0.1){
				...
				if(sunaltitude >= 0){
					...
					double inci = SolarModel.getIncidenceAngle(sunaltitude, sunazimuth, 180 , panelTit);
					double airmass = IrradianceModel.getAirMass(sunaltitude);
					...
					double res = dir + dif;
					if(inci <= 90){
						res = IrradianceModel.getIrradiancOnTiltedPlane(dir, dif, inci, sunaltitude, panelTit, 0.2);
					}else{
						res = IrradianceModel.getIrradiancOnTiltedPlane(0, dif, inci, sunaltitude, panelTit, 0.2);
					}
					...
					if(day == 113 || day == 114){
						debug.printf("%d,%f,%f,%f,%f,%f,%f,%f%n",day,time,dir,dif,res,inci,sunazimuth,sunaltitude,airmass);
					}
