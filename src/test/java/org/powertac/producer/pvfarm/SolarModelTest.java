...
public class SolarModelTest {
	...
	public void dataGetSolarPosition() throws FileNotFoundException {
		...
		for(int i = 0; i < 24;i++){
			double altitude = SolarModel.getSunAltitudeAngle(i, latitude, cal.get(Calendar.DAY_OF_YEAR));
			double azimuth = SolarModel.getSunAzinuthAngle(altitude, cal.get(Calendar.DAY_OF_YEAR), latitude,i);
	...
	public void dataGetSolarTime() throws IOException {
		...
		assertEquals(6.127, SolarModel.getSolarTime(longitude, timezone, cal.getTimeInMillis()),0.009);
	}
}
